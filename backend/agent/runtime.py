...
class AgentRuntime:
    ...
    async def start(
        ...
        *,
        ...
        # defer_advance: return as soon as the plan is persisted so the caller
        # can respond immediately and execute the steps in a background task —
        # advance() commits every step transition, so a polling client watches
        # the run progress step by step instead of getting it all at once.
        if defer_advance:
            return run
    ...
    async def approve(
        ...
        *,
        ...
        # Feedback capture: persist the payload that actually ran next to the
        # proposal, so proposed-vs-approved diffs can grade the drafts later.
        step.approved_payload = _jsonsafe(payload)
