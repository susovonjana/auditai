...
class AgentRun(Base):
    ...
    # scope of working-paper-scoped agents (procedure_buildout) — enables the
    # per-working-paper draft HISTORY list (undo/redo of any past run)
    working_paper_id: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)
    # the granted file was an audit file TEMPLATE (firm skeleton) — template
    # runs draft the firm's standard program; needed after start (synthesize
    # runs in the approve request, which doesn't carry the FE flag)
    is_template: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=False, server_default="false"
    )
    # the auditor removed this run from the draft-history list (a soft hide of
    # the run record — the sections it created are handled separately: a live
    # draft must be undone before it can be dismissed, so nothing is orphaned)
...
class AgentStep(Base):
    ...
    # feedback capture: the payload that actually ran on approval (edited or not),
    # so proposed-vs-approved diffs can measure how much auditors change drafts
    approved_payload: Mapped[Optional[dict]] = mapped_column(JSONB, nullable=True)
